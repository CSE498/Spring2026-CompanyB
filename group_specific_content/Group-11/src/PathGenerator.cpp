...
std::optional<WorldPath> PathGenerator::ShortestPath(const Point &start,
  ...
  if (std::abs(start.x - goal.x) < 0.01 && std::abs(start.y - goal.y) < 0.01) {
    WorldPath path;
    path.addPoint(start);
    return path;
  }
...
std::optional<WorldPath> PathGenerator::PatrolPath(
    const std::vector<Point> &waypoints, bool loop) {
  assert(!waypoints.empty() && "Waypoints list cannot be empty");
  ...
  for (size_t i = 0; i < waypoints.size() - 1; ++i) {
    auto segment = ShortestPath(waypoints[i], waypoints[i + 1], canMoveAnywhere);
...
WorldPath PathGenerator::ReconstructPath(
    const std::unordered_map<Point, Point, Math::PointHash> &cameFrom,
