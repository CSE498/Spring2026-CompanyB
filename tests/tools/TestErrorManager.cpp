...
TEST_CASE("ErrorManager handles warnings correctly",
  ...
  SECTION("Custom warning handler is invoked") {
    ...
    em.setWarningHandler(
        [&](const std::string& msg, ErrorManager::Severity sev) {
          handlerCalled = true;
          capturedMessage = msg;
...
TEST_CASE("ErrorManager handles fatal errors correctly",
          "[ErrorManager][fatal]") {
  ErrorManager em;
...
TEST_CASE("ErrorManager configuration", "[ErrorManager][config]") {
  ...
  SECTION("clearHandlers removes all custom handlers") {
    bool called = false;
    em.setWarningHandler(
        [&](const std::string&, ErrorManager::Severity) { called = true; });
