...
//
// Prompt: [Attached TagManager.hpp and TagManager.cpp, and TagManagerTest.cpp]
// description of how AnnotationSet works Write unit tests for the TagManager
// class
//
// Author: ChatGPT 5.2
...
// g++ -std=c++23 -Wall -I../../../third-party/Catch/single_include
// Tools/AnnotationSet.cpp Tools/TagManager.cpp Tools/TagManagerTest.cpp -o
// TagManagerTest.exe
...
static std::vector<TagManager::ObjectId> SortIds(
    std::vector<TagManager::ObjectId> v) {
  std::sort(v.begin(), v.end());
  return v;
}

static void REQUIRE_SAME_IDS(std::vector<TagManager::ObjectId> actual,
                             std::vector<TagManager::ObjectId> expected) {
...
TEST_CASE("Query multiple tags with must not contain values",
          "[QueryMultiTags]") {
  TagManager mgr;
  ...
  REQUIRE(mgr.QueryMultiTags(Tags({"Tag3"}), Tags({"Tag1", "Tag2"})).size() ==
          1);
  REQUIRE(mgr.QueryMultiTags(Tags({"Tag1", "Tag2"}), Tags({"Tag3"})).size() ==
          1);
  REQUIRE(mgr.QueryMultiTags(Tags({"Tag1"}), Tags({"Tag1", "Tag2", "Tag3"}))
              .empty());
}

// Following tests were written by AI as referenced above.
// They cover various scenarios for the TagManager class.
TEST_CASE("RegisterObject indexes tags for QueryOneTag",
          "[TagManager][RegisterObject][QueryOneTag]") {
  TagManager mgr;
...
TEST_CASE("RegisterObject overwrites old registration for same ID",
          "[TagManager][RegisterObject]") {
  TagManager mgr;
...
TEST_CASE("UnregisterObject removes object from all tag buckets",
          "[TagManager][UnregisterObject]") {
  TagManager mgr;
...
TEST_CASE("AddTag auto-registers missing objects and updates set + index",
          "[TagManager][AddTag]") {
  TagManager mgr;
...
TEST_CASE("RemoveTag updates set + index and returns false when tag missing",
...
TEST_CASE(
    "ClearTags always clears the AnnotationSet and removes index entries if "
    "registered",
...
TEST_CASE("QueryMultiTags AND behavior with required tags",
          "[TagManager][QueryMultiTags][AND]") {
  TagManager mgr;
...
TEST_CASE(
    "QueryMultiTags supports NOT list (must_not_have) when must_have is "
    "non-empty",
    "[TagManager][QueryMultiTags][NOT]") {
  TagManager mgr;
  ...
  REQUIRE_SAME_IDS(mgr.QueryMultiTags(Tags({"A", "B"}), Tags({"C", "D"})),
                   {10});
}

TEST_CASE(
    "QueryMultiTags returns empty when both must_have and must_not_have are "
    "empty",
    "[TagManager][QueryMultiTags][Edge]") {
  TagManager mgr;
...
// If must_have is empty and must_not_have is non-empty, result starts empty and
// stays empty.
TEST_CASE(
    "QueryMultiTags with empty must_have returns empty (current behavior)",
    "[TagManager][QueryMultiTags][Edge]") {
  TagManager mgr;
...
TEST_CASE("QueryOneTag returns multiple IDs for same tag",
          "[TagManager][QueryOneTag]") {
  TagManager mgr;
...
TEST_CASE(
    "AddTag on already-registered object updates QueryMultiTags AND results",
    "[TagManager][AddTag][QueryMultiTags]") {
  TagManager mgr;
...
TEST_CASE("RemoveTag removes only that tag; other tags still queryable",
          "[TagManager][RemoveTag][QueryOneTag]") {
  TagManager mgr;
...
TEST_CASE(
    "ClearTags on a non-registered object does not crash and still clears the "
    "set",
    "[TagManager][ClearTags][Edge]") {
  TagManager mgr;
...
TEST_CASE(
    "UnregisterObject on one of multiple objects sharing a tag does not remove "
    "the tag bucket for others",
    "[TagManager][UnregisterObject]") {
  TagManager mgr;
...
TEST_CASE(
    "QueryMultiTags returns empty if NOT filter removes all remaining "
    "candidates",
    "[TagManager][QueryMultiTags][NOT]") {
  TagManager mgr;
...
TEST_CASE("QueryMultiTags ignores NOT tags that do not exist in index",
          "[TagManager][QueryMultiTags][NOT]") {
  TagManager mgr;
  ...
  REQUIRE_SAME_IDS(mgr.QueryMultiTags(Tags({"A"}), Tags({"NoSuchTag"})),
                   {1, 2});
}