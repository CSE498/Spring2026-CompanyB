...
TEST_CASE("DataMap handles nested containers", "[DataMap]") {
  ...
  SECTION("Vector of pairs") {
    ...
    auto result =
        data.Get<std::vector<std::pair<int, std::string>>>("vec_pairs");
    REQUIRE(result.has_value());
...
TEST_CASE("DataMap iterator operator* returns key-value pair",
          "[DataMap][Iterator]") {
  cse498::DataMap data;
  data.Set("only", 77);
...
TEST_CASE("DataMap iterator operator-> accesses pair members",
          "[DataMap][Iterator]") {
  cse498::DataMap data;
...
TEST_CASE("DataMap iterator equality between positions",
          "[DataMap][Iterator]") {
  cse498::DataMap data;
  data.Set("a", 1);
  data.Set("b", 2);
...
TEST_CASE("DataMap const iterator As() with different types",
          "[DataMap][Iterator]") {
  cse498::DataMap mutable_data;
...
TEST_CASE("DataMap const iterator Value() returns const ref",
          "[DataMap][Iterator]") {
  cse498::DataMap mutable_data;
...
TEST_CASE("DataMap iterator AsRef mutates multiple elements",
          "[DataMap][Iterator]") {
  cse498::DataMap data;
  data.Set("a", 1);
  data.Set("b", 2);
...
TEST_CASE("DataMap iterator Value() allows mutation via any_cast",
          "[DataMap][Iterator]") {
  cse498::DataMap data;
  data.Set("x", 10);
