 ...
 * test them. Additionally use the gtest framework as I have already used
 * below."
 *
 * @date 2026-2-2
...
static_assert(
    std::is_copy_constructible<RobinHoodMap<int, int>::Iterator>::value,
...
static_assert(
    std::is_default_constructible<RobinHoodMap<std::string, double>>::value,
    "RobinHoodMap should work with string keys and double values");

static_assert(
    std::is_default_constructible<RobinHoodMap<char, std::string>>::value,
...
class RobinHoodMapTest {
 public:
  ...
  static void callResize(RobinHoodMap<K, V>& map) {
    map._resize();
  }

  template <typename K, typename V>
  static void callInsertWithHash(RobinHoodMap<K, V>& map, K key, V value,
                                 size_t hash) {
    map._insertWithHash(std::move(key), std::move(value), hash);
  }

  template <typename K, typename V>
  static size_t getTableSize(const RobinHoodMap<K, V>& map) {
    return map.mTable.size();
  }

  template <typename K, typename V>
  static size_t getInternalSize(const RobinHoodMap<K, V>& map) {
    return map.mSize;
  }
  ...
  static void addMoreItems(RobinHoodMap<int, int>& map, const int& start,
                           const int& end) {
...
TEST_CASE("Iterator dereference on valid entry does not assert",
          "[iterator-assert]") {
  RobinHoodMap<int, int> map;
  map.insert(1, 100);
  map.insert(2, 200);
...
TEST_CASE("Iterator dereference after increment stays valid",
          "[iterator-assert]") {
  RobinHoodMap<int, int> map;
  map.insert(10, 1);
  map.insert(20, 2);
...
TEST_CASE("Iterator pre-decrement skips unfilled entries",
          "[iterator-decrement]") {
  RobinHoodMap<int, int> map;
  map.insert(1, 100);
  map.insert(2, 200);
...
TEST_CASE("Iterator post-decrement returns old position",
          "[iterator-decrement]") {
  RobinHoodMap<int, int> map;
  map.insert(1, 100);
  map.insert(2, 200);
...
TEST_CASE("Iterator decrement visits only filled entries",
          "[iterator-decrement]") {
  RobinHoodMap<int, int> map;
...
TEST_CASE("Vector as value - update overwrites entire vector",
          "[data-structures]") {
  RobinHoodMap<std::string, std::vector<int>> map;
...
TEST_CASE("Set as value - duplicates in initializer are deduplicated",
          "[data-structures]") {
...
TEST_CASE("Map as value - nested map insert and retrieve",
          "[data-structures]") {
...
TEST_CASE("Deque as value - update with different size deque",
          "[data-structures]") {
...
TEST_CASE("Multiple data structure value types across maps",
          "[data-structures]") {
...
TEST_CASE("Non-const operator() <- square brackets with vector value",
          "[data-structures]") {
  RobinHoodMap<std::string, std::vector<int>> map;
