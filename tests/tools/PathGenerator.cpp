...
TEST_CASE("PathGenerator AvoidancePath routes around avoid point",
  ...
  for (const auto& p : result->pointsView()) {
    for (const auto& p : result->pointsView()) {
      double dist =
          std::hypot(p.getX() - avoid.getX(), p.getY() - avoid.getY());
      if (dist < radius - 0.1) {
  ...
  TEST_CASE("PathGenerator RandomWalk handles constrained movement",
    ...
    TEST_CASE("PathGenerator SpiralPath creates expanding pattern",
      ...
      TEST_CASE("PathGenerator SetHeuristic changes distance calculation",
        ...
        TEST_CASE("PathGenerator handles same start and goal",
                  "[pathgenerator]") {
          PathGenerator gen;
          Point start{5.0, 5.0};
          Point goal{5.0, 5.0};
        ...
        TEST_CASE("PathGenerator works with different step sizes",
                  "[pathgenerator]") {
          PathGenerator gen;
        ...
        TEST_CASE("PathGenerator EuclideanDistance computes correct distance",
                  "[pathgenerator]") {
          // This tests the static helper if it's exposed, or indirectly through
          // paths
          PathGenerator gen;
          Point start{0.0, 0.0};
          Point goal{3.0, 4.0};
        ...
        TEST_CASE(
            "PathGenerator SpiralPath with zero turns returns center only",
            "[pathgenerator]") {
          PathGenerator gen;
        ...
        TEST_CASE("PathGenerator SpiralPath with zero spacing stays at center",
          ...
          for (const auto& p : result.pointsView()) {
            for (const auto& p : result.pointsView()) {
              REQUIRE_THAT(p.getX(),
                           Catch::Matchers::WithinAbs(center.getX(), 0.01));
              REQUIRE_THAT(p.getY(),
                           Catch::Matchers::WithinAbs(center.getY(), 0.01));
            }
          }

          TEST_CASE(
              "PathGenerator AvoidancePath with zero radius finds normal path",
              "[pathgenerator]") {
            PathGenerator gen;
            Point start{0.0, 0.0};
            Point goal{5.0, 0.0};
            ...
            // radius=0 only excludes the exact avoid point, path should still
            // be found
            REQUIRE(result.has_value());
            REQUIRE_THAT(result->front().getX(),
                         Catch::Matchers::WithinRel(start.getX()));
