...
TEST_CASE("GetDestinationColour", "[TrafficWorld][accessors]") {
  ...
  CHECK(tw.GetDestinationColour(WorldPosition{999, 999})
            .empty());  // out of bounds
  CHECK(tw.GetDestinationColour(dest) ==
        tw.GetDestinationColour(dest));  // stable
...
TEST_CASE("DoAction — movement", "[TrafficWorld][movement]") {
  ...
  SECTION("all four cardinal directions work on open cross") {
    ...
    CHECK(cross.DoAction(c, 1 /*UP*/) == 1);
    c.SetLocation(centre);
    CHECK(cross.DoAction(c, 2 /*DOWN*/) == 1);
    c.SetLocation(centre);
    CHECK(cross.DoAction(c, 3 /*LEFT*/) == 1);
    c.SetLocation(centre);
    CHECK(cross.DoAction(c, 4 /*RIGHT*/) == 1);
  }
}
...
TEST_CASE("UpdateWorld — destinations",
          "[TrafficWorld][update][destinations]") {
...
TEST_CASE("RunAgents and edge cases", "[TrafficWorld][misc]") {
  ...
  SECTION("GetNumAgents starts at zero and increments per AddAgent") {
    ...
    tw.AddAgent<DrivingAgent>("A");
    CHECK(tw.GetNumAgents() == 1);
    tw.AddAgent<DrivingAgent>("B");
    CHECK(tw.GetNumAgents() == 2);
  }
  SECTION("GetAgent returns the correct agent by ID") {
    ...
    CHECK(got.GetLocation().AsWorldPosition() ==
          orig.GetLocation().AsWorldPosition());
  }
