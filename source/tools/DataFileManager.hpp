 ...
 * 
 * Eventually this class will support writing to an actual CSV file or a .o file, but for now it just buffers the data in memory.
 */
class DataFileManager {
public:
    // We use strings for the buffer because it's the standard format for CSV files which we will eventualy write to
     ...
     * * Template <typename T>: Allows passing functions that return ints, doubles, etc.
     */
    template <typename T>
    void RegisterAttribute(const std::string& columnName, std::function<T()> getter) {
