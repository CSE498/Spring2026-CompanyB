...
namespace cse498 {
     ...
     * Uses a raw function pointer instead of std::function to allow constexpr definition.
     */
    struct OperatorInfo {
        std::string_view op;
        int precedence;
    ...
    /**
     * @brief Compile-time array mapping operator strings to their corresponding OperatorInfo.
     * Prefered over std::unordered_map to avoid heap allocation and enable constexpr evaluation.
     */
    constexpr std::array<OperatorInfo, 14> OPERATORINFOMAP = {{
        {"||", -1, false, [](double a, double b) -> double { return static_cast<double>((a != 0) || (b != 0)); }},
        {"&&", 0,  false, [](double a, double b) -> double { return static_cast<double>((a != 0) && (b != 0)); }},
        {"==", 1,  false, [](double a, double b) -> double { return static_cast<double>(a == b); }},
        {"!=", 1,  false, [](double a, double b) -> double { return static_cast<double>(a != b); }},
        {">",  1,  false, [](double a, double b) -> double { return static_cast<double>(a > b); }},
        {"<",  1,  false, [](double a, double b) -> double { return static_cast<double>(a < b); }},
        {">=", 1,  false, [](double a, double b) -> double { return static_cast<double>(a >= b); }},
        ...
        {"/",  3,  false, [](double a, double b) -> double { return a / b; }},
        {"%",  3,  false, [](double a, double b) -> double { return std::fmod(a, b); }},
        {"^",  4,  true,  [](double a, double b) -> double { return std::pow(a, b); }},
    }};

    /**
     * @brief Looks up an operator entry by name with a linear scan of OPERATORINFOMAP (14 elements).
     * @param op The operator string to look up.
     * @return Pointer to the matching OperatorInfo, or nullptr if not found.
     */
    [[nodiscard]] constexpr const OperatorInfo* findOperatorEntry(std::string_view op) noexcept {
    ...
    /**
     * @brief Alias representing a compiled expression that can be evaluated given positional arguments and named variables.
     */
    using Expression = std::function<double(const std::vector<double>&, const std::map<std::string, double>&)>;
    ...
    class ExpressionParser {
        private:
            ...
            */
             ...
             */
            void shuntingYardAlgo(std::vector<Token>& sortedTokens, std::stack<Token>& operatorStack);
             ...
             * @return An Expression that takes a vector of doubles (index values) and a map of variable names to double values, and returns a double representing the result of evaluating the expression with those inputs.
             */
            [[nodiscard]] static Expression buildEvaluator(const std::vector<Token> sortedTokens);
        public:
             ...
             */
            [[nodiscard]] std::string getTokenAsString() const {
                return tokenStrings;
            }
            ...
            * @return An Expression that can be called to evaluate the expression with specific variable values and index values.
            */
