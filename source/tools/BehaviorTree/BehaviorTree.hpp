  ...
  */
  class NodeFactory {
    public:
      ...
      */
      static std::expected<std::unique_ptr<Node>, std::string> createParentNode(const NodeType& type, const std::string& name) {
        switch (type) {
      ...
      * @note The caller assumes ownership of the returned node.
      */
      static std::unique_ptr<Node> createActionNode(const std::string& name, const Action& action, const int& tickDuration) {
