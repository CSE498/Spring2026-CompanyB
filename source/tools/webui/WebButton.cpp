...
WebButton& WebButton::SetOnFileUpload(std::function<void(const std::string&)> callback) {
  ...
  if (file_input_.isNull() || file_input_.isUndefined()) {
    ...
    // Safety check: Don't try to build DOM nodes if there is no document (Headless safety)
    if (!document.isUndefined()) {
        ...
        // Bind the 'change' event (fires when the user selects a file from the OS menu)
        file_input_.call<void>(
          "addEventListener",
          std::string("change"),
          buttonHandle["onFileChange"].call<val>("bind", buttonHandle),
          config
        );
    }
  }

  // Ensure we register the button click itself so it can trigger the hidden input
  if (!on_click_) {
...
void WebButton::Click(val arg) {
  ...
  // If this button is designated for uploads, proxy the click to the hidden input!
...
void WebButton::OnFileChange(emscripten::val event) {
  ...
  auto buttonHandle = emscripten::val(std::dynamic_pointer_cast<cse498::WebButton>(shared_from_this()));
  reader.set("onload", buttonHandle["onFileReadCompleteInternal"].call<val>("bind", buttonHandle));
...
void WebButton::OnFileReadCompleteInternal(emscripten::val event) {
  ...
  // Reset the input value so the exact same file can be uploaded again if needed
  if (!file_input_.isNull() && !file_input_.isUndefined()) {
...
EMSCRIPTEN_BINDINGS(button_bindings) {
  emscripten::class_<cse498::WebButton>("WebButton")
    ...
    .function("onFileReadCompleteInternal", &cse498::WebButton::OnFileReadCompleteInternal);
}