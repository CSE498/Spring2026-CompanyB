...
WebTextbox::WebTextbox(const std::string& id, const TextStyle& style) : id(id) {
...
void WebTextbox::SetText(const std::string& text) {
  ...
  std::string safe_text = text.length() > max_length
                              ? text.substr(text.length() - max_length)
                              : text;
...
void WebTextbox::AppendText(const std::string& text) {
  ...
  if (global["document"].isUndefined()) {
    ...
    if (mock_text_content.length() > max_length) {
      mock_text_content =
          mock_text_content.substr(mock_text_content.length() - max_length);
    }
    return;
  }
...
void WebTextbox::SetMaxLength(size_t length) { max_length = length; }
