...
/**
 * @brief Internal helper to determine if the code is running without a browser window.
 * This prevents Embind from crashing when queried in a Node.js testing environment.
 */
...
/**
 * @brief Initializes the DOM element, sets its baseline styles, and appends it to the body.
 * If running headlessly, it bypasses DOM creation entirely.
 */
...
/**
 * @brief Safely removes the div element from the webpage to prevent ghost elements
 * and memory leaks when the C++ object goes out of scope.
 */
...
/**
 * @brief Replaces the contents of the textbox. Safe against cross-site scripting (XSS)
 * by utilizing innerText, and bounded by max_length_ to prevent memory issues.
 */
void WebTextbox::SetText(const std::string& text) {
    std::string safe_text = text.length() > max_length_ ? text.substr(text.length() - max_length_) : text;
...
/**
 * @brief Translates the C++ TextStyle struct into direct CSS property modifications.
 */
...
/**
 * @brief Updates the X and Y coordinates of the element relative to its container.
 */
...
/**
 * @brief Updates the truncation limit. Clamps the input to prevent developers from
 * passing 0 or values large enough to crash the browser.
 */
