...
// If the object_id is already registered, it will be unregistered first to
// prevent duplicates.
void TagManager::RegisterObject(ObjectId object_id,
                                const cse498::AnnotationSet& set) {
...
// Unregisters the object, removing it from all tag buckets and deleting its
// entry in object_tags. Returns true if the object was found and unregistered,
...
bool TagManager::UnregisterObject(ObjectId object_id) {
  ...
  // Remove object_id from all tag buckets in tag_index and clean up empty
  // buckets.
...
// Adds a tag to the object's AnnotationSet and updates the index. If the object
// is not registered, it will be registered first. Returns true if the tag was
// added, false if the tag already existed in the set.
bool TagManager::AddTag(ObjectId object_id, cse498::AnnotationSet& set,
                        const cse498::tag& tag) {
...
// Returns true if the tag was removed, false if the tag was not found in the
// set.
bool TagManager::RemoveTag(ObjectId object_id, cse498::AnnotationSet& set,
                           const cse498::tag& tag) {
...
// Clears all tags from the object's AnnotationSet and updates the index
// accordingly. If the object is not registered, it will simply clear the
// provided set.
...
// Helper function to perform AND operation on result set with another set of
// object_ids.
void TagManager::AndQueryHelper(std::unordered_set<ObjectId>& result,
                                const std::unordered_set<ObjectId>& other) {
  std::erase_if(result, [&other](const ObjectId& id) {
    return other.find(id) == other.end();
  });
}

// Helper function to perform NOT operation on result set with another set of
// object_ids.
void TagManager::NotQueryHelper(std::unordered_set<ObjectId>& result,
                                const std::unordered_set<ObjectId>& remove) {
...
std::vector<TagManager::ObjectId> TagManager::QueryOneTag(
    const cse498::tag& tag) const {
...
// Returns a list of object_ids that have all the tags in must_have and none of
// the tags in must_not_have.
std::vector<TagManager::ObjectId> TagManager::QueryMultiTags(
    const std::vector<cse498::tag>& must_have,
    const std::vector<cse498::tag>& must_not_have) const {
  ...
  // Start with the set of object_ids for the first required tag, then AND with
  // the rest.
  // If must_have is empty, start with an empty result and only apply NOT
  // filters.
  if (!must_have.empty()) {
