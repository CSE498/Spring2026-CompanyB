...
namespace cse498 {
  ...
  public:
    ...
    StateGridPosition(size_t col, size_t row, Direction direction_facing = Direction::North) // already defaults to North, but shown here for better code readability
    ...
    StateGridPosition(T col, U row, Direction direction_facing = Direction::North)
      : StateGridPosition(static_cast<size_t>(col), static_cast<size_t>(row), direction_facing) {}
    ...
    [[nodiscard]] bool IsValidForwardMove(const StateGrid& grid) const {
      // Check edge boundaries before calling GetForwardPosition to avoid size_t underflow
      if (direction_facing == Direction::North && row == 0) return false;
      if (direction_facing == Direction::West && col == 0) return false;
      StateGridPosition new_pos = GetForwardPosition();
      return grid.inBounds(static_cast<int>(new_pos.row), static_cast<int>(new_pos.col));
    }
    ...
    [[nodiscard]] bool IsValidBackwardMove(const StateGrid& grid) const {
      // Check edge boundaries before calling GetBackwardPosition to avoid size_t underflow
    ...
    [[nodiscard]] StateGridPosition GetForwardPosition() const {
      switch (direction_facing) {
        case Direction::North: 
          assert(row > 0 && "Cannot move North from row 0"); // underflows to SIZE_MAX if row == 0
          return StateGridPosition(col, row - 1, direction_facing);
        case Direction::East:  return StateGridPosition(col + 1, row, direction_facing);
        case Direction::South: return StateGridPosition(col, row + 1, direction_facing);
        case Direction::West:
          assert(col > 0 && "Cannot move West from col 0"); // underflows to SIZE_MAX if col == 0
    ...
    [[nodiscard]] StateGridPosition GetBackwardPosition() const {
      switch (direction_facing) {
        case Direction::North: return StateGridPosition(col, row + 1, direction_facing);
        case Direction::East:
          assert(col > 0 && "Cannot move East backward from col 0"); // underflows to SIZE_MAX if col == 0
          return StateGridPosition(col - 1, row, direction_facing);
        case Direction::South:
          assert(row > 0 && "Cannot move South backward from row 0"); // underflows to SIZE_MAX if row == 0
          return StateGridPosition(col, row - 1, direction_facing);
        case Direction::West:  return StateGridPosition(col + 1, row, direction_facing);
        default:
    ...
    [[nodiscard]] StateGridPosition GetOffset(long long offset_col, long long offset_row) const {
      // Asserts are for size_t underflow
      assert(static_cast<long long>(col) + offset_col >= 0 && "offset_col would underflow col");
      assert(static_cast<long long>(row) + offset_row >= 0 && "offset_row would underflow row");
