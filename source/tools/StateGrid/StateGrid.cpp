...
StateGrid::StateGrid(const std::vector<std::vector<char>> &premadeMap) {
  ...
  auto tileFromSymbol = [](char symbol, size_t r, size_t c) -> Tile {
    if (symbol == 'W') return Tile(r, c, 'W', false, "Wall", MetaData());
    if (symbol == 'P') return Tile(r, c, 'P', true, "Path", MetaData());
    throw std::invalid_argument("Only supported symbols rn are W and P");
  };
...
[[nodiscard]] bool StateGrid::inBoundsAndTraversable(size_t row,
                                                     size_t col) const {
...
bool StateGrid::moveAgent(size_t fromRow, size_t fromCol, size_t toRow,
                          size_t toCol) {
  assert(fromRow < getHeight() && fromCol < getWidth() &&
         "Source coordinates in bounds");
  assert(toRow < getHeight() && toCol < getWidth() &&
         "Destination coordinates in bounds");

  // Check preconditions before moving anything
  if (!tiles[fromRow][fromCol].hasAgent()) return false;
  if (tiles[toRow][toCol].hasAgent()) return false;
