...
void DataLog<DataClass>::AggregateData(
  ...
  if constexpr (std::is_same_v<DataClass, TrafficData>) {
    ...
    for (const auto& agent : agents) {
      ...
      const bool moved =
          has_previous && previous_it->second != state.position;

      if (has_previous && moved) {
        cumulative_distance[id] += Distance(previous_it->second, state.position);
      } else {
    ...
    for (auto it = previous_positions.begin(); it != previous_positions.end();) {
