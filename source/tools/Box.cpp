...
Box::Box(double center_x, double center_y, double half_width, double half_height)
    : center_x_(center_x), center_y_(center_y), 
      half_width_(half_width), half_height_(half_height) {
...
Point Box::GetCenter() const {
    return Point(center_x_, center_y_);
}
...
Box& Box::ScaleFromPoint(double scale_factor, double origin_x, double origin_y) {
    assert(scale_factor > 0.0 && "Scale factor must be positive");
    assert(std::isfinite(scale_factor) && "Scale factor must be finite");
...
Box& Box::Expand(double amount) {
    ...
    assert(new_half_width > 0.0 && "Expansion would result in non-positive width");
    assert(new_half_height > 0.0 && "Expansion would result in non-positive height");
...
bool Box::Overlaps(const Box& other) const {
    bool x_overlap = (center_x_ - half_width_ <= other.center_x_ + other.half_width_) &&
                     (center_x_ + half_width_ >= other.center_x_ - other.half_width_);
    
    bool y_overlap = (center_y_ - half_height_ <= other.center_y_ + other.half_height_) &&
...
std::optional<Box> Box::GetIntersection(const Box& other) const {
    ...
    double min_x = std::max(center_x_ - half_width_, other.center_x_ - other.half_width_);
    double max_x = std::min(center_x_ + half_width_, other.center_x_ + other.half_width_);
    double min_y = std::max(center_y_ - half_height_, other.center_y_ - other.half_height_);
    double max_y = std::min(center_y_ + half_height_, other.center_y_ + other.half_height_);
...
Point Box::ClosestPointTo(double x, double y) const {
    double closest_x = std::clamp(x, center_x_ - half_width_, center_x_ + half_width_);
    double closest_y = std::clamp(y, center_y_ - half_height_, center_y_ + half_height_);
...
Box Box::GetBoundingBox(const Box& other) const {
    double min_x = std::min(center_x_ - half_width_, other.center_x_ - other.half_width_);
    double max_x = std::max(center_x_ + half_width_, other.center_x_ + other.half_width_);
    double min_y = std::min(center_y_ - half_height_, other.center_y_ - other.half_height_);
    double max_y = std::max(center_y_ + half_height_, other.center_y_ + other.half_height_);
