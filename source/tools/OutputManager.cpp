...
namespace cse498 {
    ...
    void OutputManager::SetLogLevel(LogLevel level) {
        mCurrentLevel = level;
    }
    ...
    void OutputManager::LogMessage(LogLevel level, const std::string& message) {
        ...
        const bool toFile = (mCurrentLevel != LogLevel::Silent && !mOutputFilePath.empty());
        ...
        if (toFile) {
            ...
            if (mOutputStream.is_open()) {
                mBufferedLog["messages"] = mBufferedLog.value("messages", nlohmann::json::array());
                mBufferedLog["messages"].push_back({{"level", levelValue}, {"text", message}});
            }
        }
    }
