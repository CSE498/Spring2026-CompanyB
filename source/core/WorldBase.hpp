...
class WorldBase {
 ...
 public:
  ...
  AGENT_T& AddAgent(WorldData data) {
    ...
    // Provide a vector of IDs for other agents that the input agent is aware
    // of. (If not overridden, return ALL agents.)
    ...
    std::vector<size_t> GetKnownItems([[maybe_unused]] const AgentBase& agent)
        const {
      std::vector<size_t> out_ids;
