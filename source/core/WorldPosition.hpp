...
// Generated by claude, but it works so fuck it, we ball, i guess
namespace std {
  template<>
  struct hash<cse498::WorldPosition> {
    size_t operator()(const cse498::WorldPosition& pos) const noexcept {
      size_t h1 = std::hash<double>{}(pos.X());
      size_t h2 = std::hash<double>{}(pos.Y());
      return h1 ^ (h2 << 32);
    }
  };
}