...
/// A CellType might be a wall, an open space, or a hole in the ground --
/// anything that makes sense.
struct CellType {
  std::string name;  ///< Unique name for this type of cell (e.g., "wall",
                     ///< "tree", "moon")
...
/// This class provides utilities to manage, access, and modify cells within a
/// grid.
class WorldGrid {
 protected:
  ...
  std::vector<size_t> cells;  ///< Matrix of world cells, by rows, top to
                              ///< bottom; value is cell ID.
 ...
 public:
  ...
  // These are the same idea as Save and Load, but they are human readable, but
  // they also require that each state has been assigned a unique character
  // symbol.
  ...
  void Load(const std::vector<std::string>& str_grid) {
    ...
    for (size_t y = 0; y < height; ++y) {
      for (size_t x = 0; x < width; ++x) {
        ...
        cells[cell_id++] =
            (x < str_grid[y].size()) ? symbol_map[str_grid[y][x]] : 0;
      }
    }
  }
