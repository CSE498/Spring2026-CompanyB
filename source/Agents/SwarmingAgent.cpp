...
void SwarmingAgent::MergeKnowledgeFromNeighbor() {
  ...
  for (size_t agent_id : nearby) {
    ...
    // function in the AgentBase class that returns nothing then this overrides
    // it but I am unsure if we are allowed to change the AgentBase class, so
    // for now I am doing a dynamic cast to check if the other agent is a
    // SwarmingAgent and then accessing its known locations that way. This is
