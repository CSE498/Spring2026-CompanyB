...
class InfectiousWorld : public SimWorldBase {
 ...
 private:
  ...
  struct AgentHealth {
    ...
    Circle
        infection_zone;  ///< Circle around agent used for transmission checks.
  };
    ...
    for (const auto& [id, info] : health_map) {
      ...
      for (const auto& [other_id, other_info] : health_map) {
        ...
        if (zone.Contains(other_pos) && roll(rng) < transmission_rate) {
          newly_infected.push_back(other_id);
        }
      }
    }
 ...
 public:
  ...
  [[nodiscard]] size_t GetInfectedCount() const {
    size_t count = 0;
    for (const auto& [id, info] : health_map) {
      if (info.state == HealthState::INFECTED) ++count;
    }
    return count;
  }
  ...
  [[nodiscard]] size_t GetRecoveredCount() const {
    size_t count = 0;
    for (const auto& [id, info] : health_map) {
      if (info.state == HealthState::RECOVERED) ++count;
    }
    return count;
  }

  [[nodiscard]] size_t GetSusceptibleCount() const {
    return GetNumAgents() - GetInfectedCount() - GetRecoveredCount();
  }
