...
// ----------------------------------constructor----------------------------------------
// //
Text::Text(const QString& content, const QString& font, const QString& color,
           float size, bool bold, bool underline, bool italics, bool uppercase)
    : mContent(content),
      mFont(font),
      mColor(color),
      mSize(size),
...
// -------------------------------------main--------------------------------------------
// // this main function won't be in the final class implementation, it exists
...
cse498::Text byeText("bye", "Garamond", "#751401", 50, false, true, false,
false); byeText.displayText(scene);
...
/* the texts do overlap when displayed, but we are still working on the GUI so
   it is currently (or maybe completely) out of my jurisdiction to modify
   displayText to take in positions */
