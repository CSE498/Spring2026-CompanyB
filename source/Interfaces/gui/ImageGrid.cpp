...
namespace cse498
{
    ...
    ImageGrid::ImageGrid(SimWorldBase<DiseaseData> &world, QGraphicsScene &scene, int tileSize)
    ...
    void ImageGrid::RenderGrid()
    {
        ...
        for (size_t y = 0; y < grid.GetHeight(); ++y)
        {
            for (size_t x = 0; x < grid.GetWidth(); ++x)
            {
                ...
                {
                    QPixmap scaled = mImageManager.GetImage(imageID).value()
                        .scaled(mTileSize, mTileSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
