...
MainWindow::MainWindow(SimWorldBase<DiseaseData> &world, const std::vector<QString> &imagePaths,
...
void MainWindow::onFileOpen() {
    ...
    while (std::getline(input, line)) {
        if (line == "--- WORLD ---") { current = WORLD; continue; }
        if (line == "--- COMMAND LOG ---") { current = LOG; continue; }
        if (line == "--- GRAPH DATA ---") { current = GRAPH; continue; }
