...
/**
 * Stores items identified by a programmer-facing string key and a stable
 * ItemId. Supports navigation events, predicate-driven dynamic
 * visibility/enabled state, and callback hooks.
 */
class Menu {
 public:
  /**
   ...
   */
  enum class NavEvent { Up, Down, Left, Right, Activate, Back, Home, End };

  /**
   * Result of handling navigation event
   */
  enum class InputResult { Ignored, Heard };
   ...
   */
  ItemId addItem(const std::string& key, const std::string& label,
                 Action onActivate, bool enabled = true, bool visible = true);
  ...
  std::expected<void, std::string> setEnabledPredicate(ItemId id,
                                                       Predicate pred);

  /** Set an item's visible predicate. */
  std::expected<void, std::string> setVisiblePredicate(ItemId id,
                                                       Predicate pred);
