...
TrafficMainWindow::TrafficMainWindow(TrafficWorld& world,
                                     ...
                                     QWidget* parent)
    : QMainWindow(parent), mWorld(world), mImagePaths(imagePaths),
      mTileSize(tileSize), mAgentImagePath(agentImagePath)
{
...
void TrafficMainWindow::setMenuBar() {
    ...
    connect(mNewFileAction, &QAction::triggered, this, &TrafficMainWindow::onFileNew);
    ...
    connect(mOpenFileAction, &QAction::triggered, this, &TrafficMainWindow::onFileOpen);
    ...
    connect(mSaveFileAction, &QAction::triggered, this, &TrafficMainWindow::onFileSave);
    ...
    connect(mBackToMenuAction, &QAction::triggered, this, &TrafficMainWindow::onBackToMainMenu);
    ...
    connect(mExitAction, &QAction::triggered, this, &TrafficMainWindow::onFileExit);
    ...
    connect(mAboutAction, &QAction::triggered, this, &TrafficMainWindow::onHelpAbout);
    ...
    connect(mReplayToggleAction, &QAction::triggered, this, &TrafficMainWindow::onReplayToggle);
    ...
    connect(mReplayRestartAction, &QAction::triggered, this, &TrafficMainWindow::onReplayRestart);
...
void TrafficMainWindow::onTick() {
    ...
    if (mTickCount % 10 == 0) {
        ...
        logCommand(QString("[Tick %1] Active agents: %2").arg(mTickCount).arg(active));
    }
}
...
void TrafficMainWindow::redraw() {
    ...
    for (size_t y = 0; y < H; ++y) {
        for (size_t x = 0; x < W; ++x) {
            ...
            QColor colour;
            if      (mWorld.IsGrass(pos))               colour = QColor(34,  85,  34);
            else if (mWorld.HorizontalBlockedAt(pos))    colour = QColor(180, 50,  50);
            else if (mWorld.VerticalBlockedAt(pos))      colour = QColor(50,  50,  180);
            else                                         colour = QColor(80,  80,  80);
    ...
    for (size_t i = 0; i < mWorld.GetNumAgents(); ++i) {
        ...
        mGraphicsScene->addEllipse(ax + 4, ay + 4, T - 8, T - 8,
                                   QPen(Qt::NoPen), QBrush(QColor(255, 200, 0)));
    }
}
...
void TrafficMainWindow::onFileSave() {
    const QString path = QFileDialog::getSaveFileName(
        this, "Save File", QString(), "Text Files (*.txt *.sim);;All Files (*.*)");
    if (path.isEmpty()) return;
    ...
    statusBar()->showMessage(QString("File Saved: %1").arg(path), TRAFFIC_TIMEOUT);
}
